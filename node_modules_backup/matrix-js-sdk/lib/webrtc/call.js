...
class MatrixCall extends _typedEventEmitter.TypedEventEmitter {
  ...
  stopAllMedia() {
    _logger.logger.debug(!this.groupCallId ? `Call ${this.callId} stopping all media` : `Call ${this.callId} stopping all media except local feeds`);
    for (const feed of this.feeds) {
      if (feed.isLocal() && feed.purpose === _callEventTypes.SDPStreamMetadataPurpose.Usermedia && !this.groupCallId) {
        this.client.getMediaHandler().stopUserMediaStream(feed.stream);
      } else if (feed.isLocal() && feed.purpose === _callEventTypes.SDPStreamMetadataPurpose.Screenshare && !this.groupCallId) {
