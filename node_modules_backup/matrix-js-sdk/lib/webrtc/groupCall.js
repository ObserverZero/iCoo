...
class GroupCall extends _typedEventEmitter.TypedEventEmitter {
  ...
  constructor(client, room, type, isPtt, intent, groupCallId, dataChannelsEnabled, dataChannelOptions) {
    ...
    (0, _defineProperty2.default)(this, "onIncomingCall", newCall => {
      ...
      const deviceMap = (_this$calls$get = this.calls.get(opponent)) !== null && _this$calls$get !== void 0 ? _this$calls$get : new Map();
      ...
      _logger.logger.log(`GroupCall: incoming call from ${opponent.userId} with ID ${newCall.callId}`);
    ...
    (0, _defineProperty2.default)(this, "onRetryCallLoop", () => {
      ...
      for (const [member, participantMap] of this.participants) {
        ...
        for (const [deviceId, participant] of participantMap) {
          ...
          if ((call === null || call === void 0 ? void 0 : call.getOpponentSessionId()) !== participant.sessionId && this.wantsOutgoingCall(member.userId, deviceId) && retries < 3) {
    ...
    (0, _defineProperty2.default)(this, "onCallStateChanged", (call, state, _oldState) => {
      ...
      if (state === _call.CallState.Connected) {
        ...
        if ((retriesMap === null || retriesMap === void 0 ? void 0 : retriesMap.size) === 0) this.retryCallCounts.delete(opponent);
      }
    });
    (0, _defineProperty2.default)(this, "onCallHangup", call => {
      ...
      const opponent = (_call$getOpponentMemb2 = call.getOpponentMember()) !== null && _call$getOpponentMemb2 !== void 0 ? _call$getOpponentMemb2 : this.room.getMember(call.invitee);
  ...
  async create() {
    ...
    await this.client.sendStateEvent(this.room.roomId, _event.EventType.GroupCallPrefix, {
  ...
  placeOutgoingCalls() {
    ...
    for (const [member, participantMap] of this.participants) {
      ...
      const callMap = (_this$calls$get2 = this.calls.get(member)) !== null && _this$calls$get2 !== void 0 ? _this$calls$get2 : new Map();
      for (const [deviceId, participant] of participantMap) {
        const prevCall = callMap.get(deviceId);
        if ((prevCall === null || prevCall === void 0 ? void 0 : prevCall.getOpponentSessionId()) !== participant.sessionId && this.wantsOutgoingCall(member.userId, deviceId)) {
          callsChanged = true;
          if (prevCall !== undefined) {
            _logger.logger.debug(`Replacing call ${prevCall.callId} to ${member.userId} ${deviceId}`);
          ...
          if (newCall === null) {
            _logger.logger.error(`Failed to create call with ${member.userId} ${deviceId}`);
            callMap.delete(deviceId);
          } else {
            this.initCall(newCall);
            callMap.set(deviceId, newCall);
            _logger.logger.debug(`Placing call to ${member.userId} ${deviceId} (session ${participant.sessionId})`);
            newCall.placeCallWithCallFeeds(this.getLocalFeeds().map(feed => feed.clone()), participant.screensharing).then(() => {
              if (this.dataChannelsEnabled) {
                newCall.createDataChannel("datachannel", this.dataChannelOptions);
              }
            }).catch(e => {
              _logger.logger.warn(`Failed to place call to ${member.userId}`, e);
